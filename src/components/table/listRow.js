...
function ListRow({ characterData, updateCharacter, showAlert, isNoteView = false, notePreview = true, updateCharacterNote = null }) {
    ...
    const handleUpdate = (updatedCharacter) => {
        ...
        if (updateCharacterNote) {
            updateCharacterNote(updatedCharacter);
        }
    };

    useEffect(() => {
        setIsEditing(!notePreview);
    }, [notePreview]);
    ...
    {isEditing ? (
        ...
        <div className={` ${isNoteView ? 'flex flex-row' : 'p-4 bg-white dark:bg-gray-700 shadow'}`}>
            ...
            {(isExpanded || !isSmallScreen || isNoteView) && (
                <div className={`mt-2 ${isNoteView ? 'grid grid-cols-2 gap-4 w-full md:w-2/3' : 'grid grid-cols-2 gap-2 text-sm'}`}>
